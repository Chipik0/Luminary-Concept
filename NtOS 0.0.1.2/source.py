...
potrachenno_grebannyh_chasov=201
...
try:
        ...
        if check1==True:
            with open('chuck.txt') as f:
                if 'Chuck Review' in f.read():
                    logo=VideoFileClip('Media/536634/logo2.mp4')
                    logo.preview(fps=30)
                    welcome(alpha, alpha2, alpha3, sloi2, sloi3, screen, bar, barmat, unblur, blur)
        elif check2==True:
            ...
            info="Приветик"
            power1=0
            Thread(target = notify, args=(screen, info, unblur, bar, barmat, stop2, power1)).start()
    ...
    check01=os.path.exists('Media/Images/bar.png')
    check03=os.path.exists('Media/Images/barMaterialU.png')
    check06=os.path.exists('Media/Images/backunBlur.png')
    check08=os.path.exists('Cache/blurscreen.png')
    check09=os.path.exists('Comfortaa.ttf')
    check10=os.path.exists('config.txt')
    if check01==True and check03==True and check06==True and check08==True and check09==True and check10==True:
        ...
        def txt(text0, positionX, positionY, color, size):
            ...
            screen.blit(text, (positionX, positionY))
            pg.display.update()
        def lerp(a, b, t):
            return a + (b - a) * t
        def back(screen, barmat, unblur, bar):
            ...
            layer.blit(unblur, unblur.get_rect(center = screen.get_rect().center))
            layer.blit(barmat, (x, YYM))
            layer.blit(bar, (x, YYM))
        ...
        def choose_shut(screen, power1):
            ...
            pg.display.update()
        def notify(screen, info, unblur, bar, barmat, stop2,power1):
            if power1!=1:
        ...
        def errorload2(screen, white, info, Error):
                Error=1
                if info=='ram':
                    logo=VideoFileClip('Media/ram.mp4')
                    logo2=pg.image.load('Media/ram.png')
                    logo.preview(fps=30)
                    screen.blit(logo2, (0,0))
                    pg.display.update()
                if info=='cpu':
                    logo2=pg.image.load('Media/cpu.png')
                    logo=VideoFileClip('Media/cpu.mp4')
                    logo.preview(fps=30)
                    screen.blit(logo2, (0,0))
                    pg.display.update()
        ...
        def powerUSB(powerup, powerdown, screen, XM, YM, unblur, bar, barmat, blur):
                ...
                layerUSBP=pg.Surface((1920,1080))
                layerUSBP2=pg.Surface((1920,1080))
        ...
        def welcome(alpha, alpha2, alpha3, sloi2, sloi3, screen, bar, barmat, unblur, blur):
                sloi2.blit(blur, (0,0))
        ...
        def welcome2v(unblur, barmat, bar, screen, YM):
            powerlayer=pg.Surface((1920,1080))
            powerlayer2=pg.Surface((1920,1080))
            powerlayer3=pg.Surface((1920,1080))
            current_date_time = datetime.datetime.now()
            current_time = current_date_time.time()
            hour = current_time.hour
            minute = current_time.minute
            ...
            text=font.render(f"{hour:02d}:{minute:02d}", 1, (255,255,255))
            YYM=YM-53
            x, y, bla, bla2=bar.get_rect(center = screen.get_rect().center)
        ...
        sloi3=pg.Surface((1920,1080))                                 #|
        songlist=[]                                                   #|
        songlist2=[]
        XM=GetSystemMetrics(0)
        YM=GetSystemMetrics(1)                                                  #|
        sloiError=pg.Surface((1920,1080))                             #|
        sloiError2=pg.Surface((1920,1080))                            #|
        ...
        if ram<4:
            info='ram'
            errorload2(screen, white, info, Error)
